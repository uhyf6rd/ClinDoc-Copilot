...
8. 病史内容需要注意时间，患者可能把曾经的症状重述一遍，不要当作现在的症状。
...
4. **禁止**出现"未提及"、"未查"等字样
5. 不要有多余空行
6. **重点**：严禁将患者自述的“既往体温”、“在家测量的体温”、“以前查过的血压”等历史信息写入此处。此处只记录医生当前进行的体格检查结果。
7. 检查结果需要注意时间，患者可能把曾经的检查结果重述一遍，不要当作现在的检查结果。
8. 如无任何医生进行的检查结果，返回空字符串
9. **严禁**捏造对话中未提及的内容，必须根据对话事实总结
...
3. 如无检查结果，返回 "无"
4. 检查结果需要注意时间，患者可能把曾经的检查结果重述一遍，不要当作现在的辅助检查结果。
5. **严禁**捏造对话中未提及的内容，必须根据对话事实总结
...
3. 严禁进行自动推断
4. 如果医生没提建议和医嘱，**直接返回空字符串**。
5. 格式：序号 + 医嘱内容（**必须从1开始编号**）
